...
**Bottom Sheet (mobile):** draggable, Cup White, `1.5rem` top radius, pill grab handle in Frost Line colour. Tracks the finger 1:1, rubber-bands past its open/peek bounds instead of hard-stopping, and snaps to peek / half / full by projecting the release's velocity (a hard flick can skip a step) rather than a raw drag-distance threshold. Store details live here, and selecting a branch opens the sheet to full — the detail's primary action (Directions) sits in its header row, opposite the back link, so it never depends on scroll position.
...
- **Spring physics everywhere:** `stiffness: 100, damping: 20` is the default for small-amplitude motion — pins pop, buttons compress, everything has weight and bounce, like slush settling in a cup. No linear easing, ever. The bottom sheet is the one large-amplitude exception: its drag settle runs a tuned `stiffness: 500` (damping kept at the same critically-damped ratio, `damping = 2·√stiffness`, so a plain release still has zero bounce/overshoot) — simulating the plain 100/20 default against the sheet's full ~300-560px peek↔full travel showed a 0.7-1.35s settle, noticeably sluggish next to the sheet's old feel; 500 settles in ~0.5-0.7s.
...
- **Bottom sheet layout vs. transform:** the snap states are `transform: translateY()`, which is paint-only — the sheet's layout box stays its full `dvh` height, so at peek/half a large slice of it (scrollport included) lays out below the fold where nothing can be read or scrolled to. `--sheet-hidden` on `.sidebar` is the layout counterpart: `app.js` keeps it equal to the current translateY and `style.css` spends it as `padding-bottom` (and as `.detail`'s `bottom`, since `.detail` is positioned against the padding box) so the content box always ends at the visible edge. **Any change to the snap table has to keep the two in sync.** It's written only at rest, plus `0` at drag start, so the drag itself stays relayout-free.
